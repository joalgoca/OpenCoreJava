 ...
 * @author jgonzalezc
 */
@WebServlet(name = "PermisosIpServlet", urlPatterns = {"/catalogos/generals/permisosIp.do"})
public class PermisosIpServlet extends HttpServlet {
        @EJB
        CorePermisosIpFacadeLocal permisosIpEJBLocal;
        @EJB
        ...
        if(session.getAttribute(getServletContext().getInitParameter("vsi")+"SESION")==null)
        {
            ...
            try {
                if(request.getParameter("modo")==null) { 
                    int rows=10;
                    int page=1;   
                    String sort="CORE_PERMISOS_IP_ID"; 
                    String order="desc"; 
                    String filter="";
                    ...
                    response.getWriter().print(permisosIpEJBLocal.select(rows,page,sort,order,filter));                        
                }else {
                    String modo=request.getParameter("modo");
                    String id=request.getParameter("corePermisosIpId");
                    CorePermisosIp corePermisosIp=new CorePermisosIp();
                    if(modo.equals("eliminar")) {
                        permisosIpEJBLocal.remove(permisosIpEJBLocal.find(Integer.parseInt(id)));
                        ...
                        if(id!=null && !id.equals(""))
                            corePermisosIp=permisosIpEJBLocal.find(Integer.parseInt(id));                        
                        corePermisosIp.setCoreUsuarioId(usuarioEJBLocal.find(Integer.parseInt(request.getParameter("coreUsuarioId"))));                        
                        corePermisosIp.setAccion(request.getParameter("accion").toUpperCase());
                        corePermisosIp.setIp(request.getParameter("ip"));
                        corePermisosIp.setEstatus(request.getParameter("estatus"));
                        response.getWriter().print(permisosIpEJBLocal.persistir(corePermisosIp,modo));                          
                    }
                }   
    ...
    private String fieldParse(String campo){
            switch (campo) {
                case "corePermisosIpId":
                    return "CORE_PERMISOS_IP_ID";
                case "coreUsuarioId":
